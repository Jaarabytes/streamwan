...
export default function Packages() {
    ...
    const config = { email: 'xh3rking96@gmail.com', currency: "KES", publicKey: publicKey, amount: 2000 * 100 }
    ...
    <div className="grid md:grid-cols-4 gap-4 px-5 min-h-full sm:grid-cols-2 grid-cols-1 mb-10">
      {items.map((elem) => (
        <div key={elem.name} className={`rounded-lg border-gray-800 shadow-lg py-5`}>
          ...
          <hr />
          <p className="my-5 text-black px-3 text-lg"><b>{elem.price}</b> KES per month</p>
          <hr className="my-5" />
